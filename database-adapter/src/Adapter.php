...
class Adapter implements AdapterContract, FilteredAdapterContract, BatchAdapterContract, UpdatableAdapterContract
{
    ...
    {
        if($_SESSION["user_id"]) {
            $row = $this->connection->query('SELECT v1 FROM ' . $this->casbinRuleTableName . ' WHERE ptype="g" AND v0=' . $_SESSION["user_id"] . ' AND v2="active"');
            $user_role = $row[0]['v1'] ?? 'employee';
            $rows = $this->connection->query('SELECT ptype, v0, v1, v2, v3, v4, v5 FROM ' . $this->casbinRuleTableName . ' WHERE (ptype="p" and v0="' . $user_role . '") OR (ptype="g" AND v0="' . $_SESSION["user_id"] . '")');
        } else {
